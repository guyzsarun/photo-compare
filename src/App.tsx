...
function App() {
  ...
  // Update canvas dimensions based on window resize and panel state
  useEffect(() => {
    const updateDimensions = () => {
      // Panel is 320px (w-80) when expanded, 32px (w-8) when collapsed, side padding is 16*2
      const panelWidth = markerPanelCollapsed ? 32 : 320;
      const availableWidth = window.innerWidth - panelWidth - 32;
  ...
  return (
    <div className="flex flex-col h-screen bg-stone-100 dark:bg-slate-900 text-stone-800 dark:text-slate-100 font-sans overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <MarkerPanel collapsed={markerPanelCollapsed} onToggle={() => setMarkerPanelCollapsed(c => !c)} />
      </div>
    </div>
  );
}
